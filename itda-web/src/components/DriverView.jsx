...
export default function DriverView() {
  ...
  // 우선순위는 수요예측 결과와 연동 가능(현재는 데모 값)
  const sampleVillages = [
    { id: 1, lat: 35.284, lon: 126.514, priority: 0.9, service_min: 8 },
    { id: 2, lat: 35.3,  lon: 126.488, priority: 0.7, service_min: 8 },
    { id: 3, lat: 35.27, lon: 126.53,  priority: 0.5, service_min: 8 },
  ];
  ...
  const handleOptimize = async () => {
    ...
    try {
      const data = await postJSON("/route/optimize", {
        vehicle: { start_lat: 35.271, start_lon: 126.502, start_time: `${new Date().toISOString().slice(0,10)}T09:00:00` },
        villages: sampleVillages,
        objective: "weighted_distance_time",
        meta: { source: "driver-view" },
      });
      setRouteResult(data);
      setDriverPage("route");
    } catch (e) {
  ...
  const NavItem = ({ id, icon, label, onClick }) => (
    <li
      ...
      onClick={() => (onClick ? onClick() : setDriverPage(id))}
    ...
    <div className="driver-container">
      ...
      <div className="driver-sidebar">
        ...
        <ul className="driver-nav">
          ...
          {/* 사이드 위젯: 음성 버튼을 누르면 음성 페이지로 이동 */}
          <div className="voice-widget" style={{ marginTop: 24 }}>
            <h4>음성 도우미</h4>
            <button
              className="voice-button-main"
              onClick={() => setDriverPage("voice")}
              title="음성 도우미 열기"
              aria-label="음성 도우미 열기"
            >
              🎤
            </button>
            <p style={{ fontSize: 13, opacity: 0.9 }}>클릭하여 음성 도우미 열기</p>
          </div>
        </ul>
      </div>
      ...
      <div className="driver-main">
        ...
        <div className="driver-content">
          ...
          {driverPage === "home" && (
            <div className="stats-grid">
              <div className="stat-card">
                ...
                <div className="stat-label">방문 예정 마을</div>
              </div>
              <div className="stat-card">
                <div className="stat-number">2h 15m</div>
                <div className="stat-label">예상 소요시간</div>
              </div>
              <div className="stat-card">
                <div className="stat-number">₩337K</div>
                <div className="stat-label">예상 총 매출</div>
              </div>
            </div>
          )}
                  ...
                  <div className="table-row" key={idx}>
                    <div>
                      <input
                        ...
                        onChange={(e)=>changeCell(idx,"product_id",e.target.value)}
                        placeholder="예: 101"
                      />
                    </div>
                    <div>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      ...
                      <input
                        ...
                        value={row.qty}
                        onChange={(e)=>changeCell(idx,"qty",e.target.value.replace(/[^\d]/g,""))}
                      />
                      <button className="qty-btn" onClick={()=>stepQty(idx,+1)}>+</button>
                    </div>
          ...
          {/* 음성 도우미: VoiceAgent 포함 */}
          {driverPage === "voice" && (
            <div className="card">
              <div className="card-title">🎤 음성 도우미 (Agent)</div>
              {/* VoiceAgent 자체가 STT/TTS/도구호출/로그를 모두 포함 */}
              <VoiceAgent />
              <div className="alert alert-info" style={{ marginTop: 12 }}>
                <b>예시 명령:</b> “경로 계산해”, “경로 요약”, “알림 읽어줘”, “현재 시간”, “예측 실행”, “재고 최적화 추천”
              </div>

              {/* 참고용 최근 알림 */}
              <button className="button button-secondary" onClick={fetchLatestAlert} style={{ marginTop: 8 }}>
                🔄 최근 알림 불러오기
              </button>
              {latestAlert && (
